...
# we exclude Dyas_from_mfg as there is no overlap between train and test data
...
# we will skip using this variable in model fit
imputed_test_data[,Days_from_mfg := NULL]
test_data[,Days_from_mfg := NULL]
...
## Obtaining weight for class 0 to overcome imbalance in data
```{r,}
...
```{r, cross validation and hyperparameter tuning}
...
xgb_cv_bayes <- function(max_depth, min_child_weight, subsample, nrounds) {
  cv <- xgb.cv(params = list(booster = "gbtree", eta = 0.01,
                             ...
                             eval_metric = "auc", scale_pos_weight=weight_0),
...
xgb1 <- xgboost (params = params_list, data = dtrain, nrounds = 28,
...
optCutOff <- optimalCutoff(validation_data$Target, xgbpred)[1] 

optCutOff

xgbpred <- ifelse (xgbpred > optCutOff, 1, 0)


library(caret)
caret::confusionMatrix(as.factor(as.numeric(validation_data$Target)), as.factor(xgbpred))
```
...
test_mat <- model.matrix(~.+0,data = test_data[, !names(test_data) %in% "Cat_M"])
test_dmatrix <- xgb.DMatrix(data = test_mat)

# Predicting for test data
test_xgbpred <- predict(xgb1,test_dmatrix)

## FINAL SUBMISSION FILE
# Saving predictions for Test data for submission
test_ids <- read.csv("../Data/Ineight-Analyst-Testing-Data.csv")[,1]
write.csv(cbind(test_ids, test_xgbpred), "../submission_xgb.csv", row.names = F)
