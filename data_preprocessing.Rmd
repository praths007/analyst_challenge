...
continuous_imputed_train_data <- cbind(imputed_train_data, train_data[,.SD,.SDcols = c("Target", "ID")])
...
continuous_imputed_train_data <- data.table(continuous_imputed_train_data)
# temp changing factors to characters for substituting NAs with Others
nominal_vars_to_subset <- nominal_vars %nin% columns_to_remove


temp_impute <- continuous_imputed_train_data[, lapply(.SD, as.character), .SDcols=c(nominal_vars_to_subset)]


replace_blanks_with_others <- function(x){
  if((continuous_imputed_train_data=="") | (continuous_imputed_train_data=="NA") | (continuous_imputed_train_data=="na")){
    return("Others")
  }
...
continuous_imputed_train_data[] <- "Others"
