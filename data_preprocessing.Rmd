...
# Flag, Count_2 and Count_3 have ~67% missing/NULL values
...
qplot(data = train_data[,.(count=.N), by=c("Cat_A")], x=Cat_A, y=count, geom = "density")
qplot(data = train_data[,.(count=.N), by=c("Cat_B")], x=Cat_B, y=count, geom = "density")
qplot(data = train_data[,.(count=.N), by=c("Cat_C")], x=Cat_C, y=count, geom = "density")
qplot(data = train_data[,.(count=.N), by=c("Cat_D")], x=Cat_D, y=count, geom = "density")
qplot(data = train_data[,.(count=.N), by=c("Cat_E")], x=Cat_E, y=count, geom = "density")
qplot(data = train_data[,.(count=.N), by=c("Cat_F")], x=Cat_F, y=count, geom = "density")
qplot(data = train_data[,.(count=.N), by=c("Cat_G")], x=Cat_G, y=count, geom = "density")
qplot(data = train_data[,.(count=.N), by=c("Cat_H")], x=Cat_H, y=count, geom = "density")
qplot(data = train_data[,.(count=.N), by=c("Cat_I")], x=Cat_I, y=count, geom = "density")
qplot(data = train_data[,.(count=.N), by=c("Cat_J")], x=Cat_J, y=count, geom = "density")
qplot(data = train_data[,.(count=.N), by=c("Cat_K")], x=Cat_K, y=count, geom = "density")
qplot(data = train_data[,.(count=.N), by=c("Cat_L")], x=Cat_L, y=count, geom = "density")
qplot(data = train_data[,.(count=.N), by=c("Cat_M")], x=Cat_M, y=count, geom = "density")
qplot(data = train_data[,.(count=.N), by=c("Cat_N")], x=Cat_N, y=count, geom = "density")
...
count_above_98th_percentile <- function(x){
  length(which(x > quantile(x, c(0.98), na.rm = TRUE)))
  
}

count_below_2th_percentile <- function(x){
  length(which(x < quantile(x, c(0.02), na.rm = TRUE)))
  
}

count_between_iqr <- function(x){
  length(which((x > quantile(x, c(0.25), na.rm = TRUE)) & (x < quantile(x, c(0.75), na.rm = TRUE))))
}

print("count observations above 98th percentile")
train_data[,lapply(.SD, count_above_98th_percentile), .SDcols = c(continuous_vars, ordinal_vars)]

print("count observations below 5th percentile")
train_data[,lapply(.SD, count_below_2th_percentile), .SDcols = c(continuous_vars, ordinal_vars)]

print("count observations between 25th nd 75th percentile")
train_data[,lapply(.SD, count_between_iqr), .SDcols = c(continuous_vars, ordinal_vars)]

# because the data is exponentially distributed as we have seen above we observe that
# there are a sizeable number of observations that lie outside the IQR, we cannot treat them as outliers
```
...
(train_data[,lapply(.SD,),.SDcols=c("Cat_A")], is.na(x))


sapply(train_data, function(X){ x==""})

print(train_data[, lapply(.SD, function(X){length(x == "")}), .SDcols=c(continuous_vars, ordinal_vars, "Flag")])
